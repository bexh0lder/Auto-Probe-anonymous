# evaluate_mplugowl2.py
# ========================================================================================
# 该脚本是 evaluate_mplugowl.py 的一个适配版本，专门用于 mPLUG-Owl2 模型。
# 它保留了原脚本的所有评估框架、问题处理逻辑、统计和报告功能，
# 仅将模型调用核心替换为 mPLUG-Owl2 的方式。
#
# 主要变化:
# 1. 使用 mplug_owl2.model.builder.load_pretrained_model 加载模型。
# 2. 推理函数中增加了图像预处理（缩放）和对话式提示构建流程。
# 3. 适配 mPLUG-Owl2 的 model.generate() 调用方式。
# 4. 答案解析、统计计算等上层逻辑保持不变。
# ========================================================================================
...
# ### 核心改动 ###: 用户需要根据自己的环境调整 mPLUG-Owl2 的库路径
sys.path.append("/root/Auto-Probe/mPLUG-Owl/mPLUG-Owl2") # Example from notebook

# ### 核心改动 ###: 导入 mPLUG-Owl2 相关模块
...
def parse_args():
    ...
    """
    parser = argparse.ArgumentParser(description="在TIDE数据集上评估 mPLUG-Owl2 模型")
...
def load_and_merge_config(cli_args):
    """
    从YAML配置文件加载参数，并与CLI参数合并（CLI优先）- 适配 mPLUG-Owl2
    """
    ...
    args.model_name = model_config.get('name', get_model_name_from_path(args.model_path) if args.model_path else 'mplug_owl2_model')
    ...
    args.output_filename = cli_args.output_filename if cli_args.output_filename else data_config.get('output_filename', 'tide_evaluation_results_owl2.json')
...
def setup_logger(output_dir, log_level='INFO'):
    logger = logging.getLogger("tide_mplugowl2_evaluator") # 改名
    ...
    log_file = os.path.join(output_dir, "tide_evaluation_mplugowl2.log") # 改名
...
# --- 核心改动：新的推理函数 ---
def ask_mplugowl2_safe(system_prompt, user_question, image, model, tokenizer, image_processor, args, logger=None):
    """
    安全地调用 mPLUG-Owl2 模型进行推理
    """
    try:
        # 1. 图像预处理
        ...
        inp = DEFAULT_IMAGE_TOKEN + user_question
        ...
        stop_str = conv.sep2 if conv.sep2 is not None else conv.sep
        keywords = [stop_str]
        ...
        with torch.inference_mode():
            output_ids = model.generate(input_ids, images=image_tensor, **generate_kwargs)
        
        # 5. 解码
        raw_output = tokenizer.decode(output_ids[0, input_ids.shape[1]:]).strip()
        cleaned_output = raw_output.replace(stop_str, "").strip()
        
        return cleaned_output
        
    except Exception as e:
        if logger: logger.error(f"mPLUG-Owl2 推理过程中出错: {e}", exc_info=True)
        return f"ERROR: {str(e)}"

# --- 核心改动：适配 mPLUG-Owl2 的问题处理函数 ---
def process_basic_question(question_item, image_dir, model, tokenizer, image_processor, args, logger=None):
    """处理基础Yes/No问题 (适配 mPLUG-Owl2)"""
    ...
    try:
        ...
        # 为 mPLUG-Owl2 构建 System Prompt 和 User Question
        system_prompt_for_model = "You are a helpful assistant. Answer the question with only the word YES or the word NO."
        user_question_for_model = question_text
        
        raw_output = ask_mplugowl2_safe(system_prompt_for_model, user_question_for_model, image, model, tokenizer, image_processor, args, logger)
        
        # --- START: 答案解析逻辑 (与原版完全相同) ---
...
def process_enhanced_question(question_item, image_dir, model, tokenizer, image_processor, args, logger=None):
    """处理增强选择题问题 (适配 mPLUG-Owl2) - 开放式回答版本"""
    image_file = question_item.get("image")
    if not image_file:
    ...
    try:
        ...
        # 为 mPLUG-Owl2 构建 System Prompt 和 User Question - 开放式回答
        system_prompt_for_model = "You are a helpful assistant. Please analyze the image and provide a detailed answer to the question."
        user_question_for_model = question_text
...
# ### 核心改动 ###: 更新函数签名以传递 mPLUG-Owl2 相关对象
...
def main():
    ...
    try:
        ...
        logger.info("TIDE数据集 mPLUG-Owl2 评估开始")
        ...
        # ### 核心改动：加载 mPLUG-Owl2 模型 ###
        logger.info("开始加载 mPLUG-Owl2 模型...")

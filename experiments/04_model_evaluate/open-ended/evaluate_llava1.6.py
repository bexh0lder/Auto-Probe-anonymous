...
# ### Core change ###: Import LLaVA 1.6 related modules
from transformers import LlavaNextProcessor, LlavaNextForConditionalGeneration

def parse_args():
    """
    Parse command line arguments, support CLI override YAML configuration
    """
    ...
    parser.add_argument("--tide_dataset_file", type=str, default=None, help="TIDE dataset file path (override YAML)")
    parser.add_argument("--image_dir", type=str, default=None, help="Image directory path (override YAML)")
    parser.add_argument("--output_dir", type=str, default=None, help="Output directory path (override YAML)")
    parser.add_argument("--output_filename", type=str, default=None, help="Output filename (override YAML)")
    parser.add_argument("--summary_csv", type=str, default=None, help="CSV file path for recording evaluation result summary (override YAML)")
    
    # ### Core change ###: Model parameters adapted for LLaVA 1.6
    parser.add_argument("--model_path", type=str, default=None, help="Model path (override YAML)")
...
def load_and_merge_config(cli_args):
    """
    Load parameters from YAML configuration file and merge with CLI arguments (CLI takes priority) - adapted for LLaVA 1.6
    """
...
def process_enhanced_question(question_item, image_dir, model, processor, args, logger=None):
    """Process enhanced multiple choice questions (adapted for LLaVA 1.6) - open-ended answer version"""
    image_file = question_item.get("image")
    if not image_file:
        return {**question_item, "prediction": "ERROR: Missing image filename", "predicted_answer": None, "correct": None}
    
    image_path = os.path.join(image_dir, image_file)
    if not os.path.exists(image_path):
        return {**question_item, "prediction": "ERROR: Image file does not exist", "predicted_answer": None, "correct": None}
    
    try:
        image = Image.open(image_path).convert('RGB')
        question_text = question_item.get("text", "")
        
        # Build conversational prompt for LLaVA 1.6 - open-ended answer
        system_prompt = "You are a helpful assistant. Please analyze the image and answer the question."
        conversation = [
            {"role": "system", "content": [{"type": "text", "text": system_prompt}]},
        ...
        # For open-ended answers, do not parse the answer
        if raw_output.startswith("ERROR:"):
